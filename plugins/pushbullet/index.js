...
var settings = _.extend(require('../../conf/config'), require('./config'));
var PushBullet = require('pushbullet');
var pusher = new PushBullet(settings.config.pushbullet_key);
var moment = require('moment');

function getMethod(req, res) {
  res.writeHead(200, {'Content-Type': 'text/plain'});
  res.end('GET call for ' + settings.pluginConfig.name + ' is not supported. Use POST instead');
}

function postMethod(req,res) {
  pusher.devices(function(error, response) {
    if (settings.config.device_id !== undefined) {
      sendMessage(req.body, settings.config.device_id);
    } else {
...
function sendMessage(data, device_id) {
  ...
  var image_to_send;
  if (settings.config.image_method == 'URL') {
    image_to_send = settings.config.images_base_url + receivedData.pathToImage;
  } else if (settings.config.image_method == 'PATH') {
    image_to_send = settings.config.images_base_path + receivedData.pathToImage;
  }
